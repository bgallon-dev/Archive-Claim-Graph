...
Interactive (recommended — password not visible in shell history):
    ...
    python -m gemynd.auth.seed --email admin@example.com

Non-interactive (CI / Docker entrypoint):

    python -m gemynd.auth.seed --email admin@example.com --password "s3cur3P4ssw0rd!"

Also registered as a CLI entry point ``gemynd-create-admin`` in pyproject.toml.
"""
