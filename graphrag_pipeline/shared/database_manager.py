...
def _migrate_users(conn: sqlite3.Connection) -> None:
    from graphrag_pipeline.auth.store import _SCHEMA, _MIGRATE_TOKEN_VERSION
...
def _migrate_review(conn: sqlite3.Connection) -> None:
    from graphrag_pipeline.review.store import _DEFAULT_ANTI_PATTERNS, _SCHEMA_SQL
...
def _migrate_annotation(conn: sqlite3.Connection) -> None:
    from graphrag_pipeline.ingest.annotation.store import _SCHEMA_SQL
...
def _migrate_token_usage(conn: sqlite3.Connection) -> None:
    from graphrag_pipeline.shared.token_tracker import _SCHEMA_SQL

    conn.executescript(_SCHEMA_SQL)


def _migrate_write_audit(conn: sqlite3.Connection) -> None:
    from graphrag_pipeline.retrieval.web.write_audit_log import (
        _CREATE_INDEX,
        _CREATE_TABLE,
    )
